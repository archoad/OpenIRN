...
| Système d'information (SI) | ensemble de moyens qui soutient une fonction critique |
...
1. Ouvrir OpenIRN.
2. Sélectionner **Choisir un espace de travail**.
3. Choisir l'espace communiqué par votre responsable.
4. Vérifier son nom avant de continuer.
...
1. Ouvrir **Autoriser ce terminal**.
2. Si vous possédez un code à usage unique, le saisir.
3. Sinon, sélectionner **Demander une autorisation**.
4. Informer le Pilote IRN ou l'administrateur de l'espace.
5. Lorsque vous recevez un code approuvé, le saisir avant son expiration.

Ne pas envoyer le code dans un canal public. Une autorisation concerne un terminal et un espace précis.

## Ouvrir une session

1. Sélectionner **Déverrouiller OpenIRN**.
2. Choisir votre profil.
3. Saisir votre code personnel.
4. Si le code est temporaire, choisir immédiatement un nouveau code.
...
Sélectionner le drapeau affiché dans la barre supérieure, puis choisir la langue. Les langues disponibles dans l'application actuelle sont français, anglais, espagnol et allemand.
...
Si une carte n'apparaît pas, vérifier d'abord votre rôle. L'interface masque les fonctions interdites et le serveur contrôle également chaque opération sensible.

# 5. Consulter le référentiel officiel

1. Depuis l'accueil, ouvrir **Référentiel aDRI IRN**.
2. Consulter les piliers proposés.
3. Ouvrir un pilier pour afficher ses critères.
4. Ouvrir un critère pour lire son libellé, sa description, son périmètre, ses recommandations et ses références.
...
1. Sélectionner l'ajout d'une fonction critique.
2. Lui donner un nom métier compréhensible.
3. Ajouter une description qui explique les conséquences d'une interruption.
4. Enregistrer.
...
1. Vérifier que le référentiel officiel est installé.
...
La campagne débute au statut **Brouillon**. Éviter les noms génériques comme « Test » sur une instance de production.

# 8. Affecter les critères

1. Ouvrir la campagne.
2. Ouvrir **Affectations des critères**.
3. Choisir un pilier puis un critère.
4. Sélectionner l'évaluateur compétent.
5. Enregistrer l'affectation.

Répartir les critères selon la connaissance réelle des actifs, pas seulement selon la disponibilité. Un évaluateur ne peut saisir que ce qui lui est explicitement affecté.

# 9. Répondre à une évaluation

## Choisir l'actif

Lorsqu'une campagne contient plusieurs actifs, sélectionner d'abord l'actif à évaluer. Les réponses sont distinctes par actif.
...
Observer l'indicateur de synchronisation après une modification. En cas d'erreur, ne pas effectuer plusieurs changements contradictoires sur différents terminaux. Noter l'heure, conserver le message et prévenir le Pilote IRN.

# 10. Utiliser le contrôle qualité

Ouvrir **Contrôle qualité** depuis la campagne. L'écran met en évidence notamment :
...
Traiter les anomalies avant de passer au statut **Prêt pour revue**. Un score ne remplace pas la qualité des preuves.
...
Une valeur très faible peut donc peser fortement sur le résultat. Toujours lire la synthèse avec les réponses et justifications associées.

L'écran peut produire des exports PNG et PDF de synthèse. Vérifier le périmètre, la campagne, l'actif et la date avant diffusion.
...
Le Pilote IRN, le validateur ou l'administrateur agit selon les commandes proposées par l'interface et ses droits. Avant validation, vérifier le contrôle qualité, le périmètre d'actifs, les justifications et la synthèse.
...
1. ouvrir le fichier ;
2. vérifier le nom de la campagne et l'espace ;
3. vérifier qu'aucune information non destinée au destinataire n'y figure ;
4. utiliser le canal de partage approuvé par l'organisation.
...
1. Vérifier l'espace affiché.
2. Ajouter ou modifier le profil.
3. Attribuer le rôle strictement nécessaire.
4. Activer ou désactiver le compte.
5. Enregistrer.
...
Lors d'un départ, désactiver le compte et demander à l'administrateur de révoquer ses sessions actives.
...
Avant d'approuver, vérifier l'identité du demandeur par un canal distinct. En cas de perte ou de vol, révoquer le terminal avant toute autre action.

# 17. Changer ou verrouiller sa session

Pour changer votre code, ouvrir **Administration → Changement de code** lorsque cette carte est disponible, saisir le code actuel puis deux fois le nouveau.

Choisir un code non trivial, différent du précédent et non partagé. OpenIRN refuse les suites évidentes et les répétitions.

Verrouiller l'application lorsque vous quittez le terminal. Après verrouillage, le terminal reste autorisé mais aucune opération utilisateur sensible n'est permise avant une nouvelle authentification.
...
- attribuer le rôle le plus limité compatible avec la mission ;
- nommer clairement espaces, SI, actifs, campagnes et terminaux ;
- demander une justification pour chaque réponse significative ;
- réaliser les essais dans une campagne de recette identifiée ;
- ne jamais partager un code PIN, un code d'enrôlement ou une capture contenant des secrets ;
- verrouiller la session avant de laisser le terminal ;
- signaler immédiatement une perte de terminal ;
- exporter uniquement les données nécessaires ;
- faire valider la campagne par une personne différente de l'évaluateur lorsque l'organisation le permet.
