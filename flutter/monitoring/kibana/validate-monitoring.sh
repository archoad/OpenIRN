...
jq -e '
	...
	.time_range == {"from": "now-3d/d", "to": "now"} and
	...
	([.panels[] | select(.type == "markdown" and (.config.content | startswith("## ")) and ((.config.content | startswith("## Performance")) | not) and .grid.h == 4)] | length == 5) and
	([.panels[] | select(.type == "markdown" and (.config.content | startswith("## Performance")) and .grid == {"x": 0, "y": 45, "w": 48, "h": 7})] | length == 1) and
	...
	([.panels[] | select(.config.type == "metric")] | length == 23) and
	(all(.panels[] | select(.config.type == "metric");
		(.config | has("title") | not) and
		(.config.metrics | length == 1) and
		(.config.metrics[0].type == "primary") and
		(.config.metrics[0].label | type == "string" and length > 0)
	)) and
